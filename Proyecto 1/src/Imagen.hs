...
subImagen xInicial yInicial anchura' altura' (Imagen _ _ pixels) = 
    Imagen anchura' altura' (map (take (fromInteger anchura') . drop (fromInteger xInicial)) filas)
    where
      filas = take (fromInteger altura') (drop (fromInteger yInicial) pixels)
--subImagen xInicial yInicial anchura' altura' (Imagen _ _ pixels) = 
...
hSplit (Imagen anchura' altura' pixels) = 
        (subImagen 0 0 anchura' (quot altura' 2) (Imagen anchura' altura' pixels), 
        subImagen 0 (quot altura' 2) anchura' (quot altura' 2) (Imagen anchura' altura' pixels))
        --(subImagen 0 (quot altura' 2) anchura' (quot altura' 2) (Imagen anchura' altura' pixels), 
        --subImagen 0 0 anchura' (quot altura' 2) (Imagen anchura' altura' pixels))
...
colorPromedio (Imagen anchura' altura' pixels) =
    Color {rojo = fromInteger (rojoSuma colores `div` cantidad),
          verde = fromInteger (verdeSuma colores `div` cantidad),
          azul = fromInteger (azulSuma colores `div` cantidad)}
    where
      colores = concat pixels
      cantidad = anchura' * altura'
